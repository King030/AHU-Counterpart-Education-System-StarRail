...
def project_data_upd(request):
    ...
    models.Projects.objects.filter(id=request.POST.get('id')).update(location=request.POST.get('location'))
    models.Projects.objects.filter(id=request.POST.get('id')).update(time=request.POST.get('time'))
    return success()
...
def select_data_page(request):
    ...
    for item in list(paginator.page(pageIndex)):
        ...
        temp = {
            ...
            'location': item.workPaln.project.location,
...
def student_selected_score_view(request):

    return render(request, "StudentScore.html")

def select_data_student_selected_score(request):
...
def teachers_score_add(request):#点击考试成绩添加之后返回的页面

    return render(request, "ScoreAdd.html")

def select_stu_score_for_teacher(request):#考试成绩添加里面的信息
    ...
    for item in list(paginator.page(pageIndex)):
        ...
        temp = {
            'id': item.id,
            'plan_id': item.workPaln.project.id,
            'plan_name': item.workPaln.project.name,
            'student_id': item.student.user.id,
            'student_name': item.student.user.name,
            'score': item.score,
            'studentColleges': item.student.college.name,
            'studentMajors': item.student.major.name,
        }
        resl.append(temp)
    ...
    return successData(temp)
def tea_add_score(request):

    project = models.Projects.objects.get(name=request.POST.get('plan_name'))
    user = models.Users.objects.get(name=request.POST.get('student_name'))
    student = models.Students.objects.get(user=user)
    # models.SelectLogs.objects.create(id=int(time.time()),
    #                                  student=student,
    #                                  workPaln=models.WorkPalns.objects.get(project=project),
    #                                  score=request.POST.get('score'),
    #                                  )
    models.SelectLogs.objects.filter(student=student, workPaln=models.WorkPalns.objects.get(project=project)).update(score=request.POST.get('score'))
    return success()
...
def testLocation_view(request):
    ...
    if user['type'] == 0:
        ...
        grades = models.Grades.objects.all().values()
        projects = models.Projects.objects.all().values()
        testlocation = models.TestLocation.objects.all().values()

        return render(request, "adminTestLocation.html", {'grades': grades, 'projects': projects, 'testlocation': testlocation})
    elif user['type'] == 1:

        grades = models.Grades.objects.all().values()
        return render(request, "teacherTestLocation.html", {'grades': grades})
    elif user['type'] == 2:

        return render(request, "teacherTestLocation.html")

def testLocation_data_info(request):
...
def testLocation_data_page(request):
    pageIndex = request.GET.get('pageIndex', 1)
    pageSize = request.GET.get('pageSize', 10)
    teacherName = request.GET.get('teacherName')
    ...
    if isExit(teacherName):
        qruery = qruery & Q(testlocation__workpalns__teacher__user__name__contains=teacherName)



    data = models.TestLocation.objects.filter(qruery)
    ...
    for item in list(paginator.page(pageIndex)):
        temp = {
            ...
            'time': item.time,
            'location': item.location,
            'name': item.workPaln.teacher.user.name,
        }
        resl.append(temp)
...
def testLocation_data_add(request):

    user = models.Users.objects.get(name=request.POST.get('name'))
    teacher = models.Teachers.objects.get(user=user)
    project = models.Projects.objects.get(name=request.POST.get('projectName'))
    workPlan = models.WorkPalns.objects.get(teacher=teacher, project=project)
    models.TestLocation.objects.create(id=int(time.time()),workPaln=workPlan,location=request.POST.get('location'),
                                       time=request.POST.get('time'))

    return success()

def testLocation_data_upd(request):
    ...
    project = models.Projects.objects.get(name=request.POST.get('projectName'))
    workPlan = models.WorkPalns.objects.get(teacher=teacher,project=project)
    models.TestLocation.objects.filter(
        workPaln=workPlan
    ).update(
            location=request.POST.get('location'),
            time=request.POST.get('time'))
...
def testLocation_data_del(request):
    models.TestLocation.objects.filter(id = request.POST.get('id')).delete()
    return success()

def testLocation_data_page_teacher(request):
    pageIndex = request.GET.get('pageIndex', 1)
    pageSize = request.GET.get('pageSize', 10)
    teacherName = request.GET.get('teacherName')
